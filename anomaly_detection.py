...
def f2_score(scores):
    beta = 2
    anomalies = np.argwhere(scores < np.percentile(scores, 27.5)).flatten()
...
ump = UMAP(n_components=2, n_neighbors=100).fit_transform(X)
plot_ddr(ump, y)

def save_algorithm(file_path, algorithm):
...
save_algorithm('lof.pkl', lof_best)
...
save_algorithm('ocsvm.pkl', ocsvm_best)
